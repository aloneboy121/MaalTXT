...
class Config(object):
    ...
    API_ID = int(os.environ.get("API_ID",  "23480585")
   API_HASH = os.environ.get("API_HASH", "23af64c0a4ba8c1f0c04e9be07d0ba68")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '7306229091').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002247529229').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = -1002190206859
...
if __name__ == "__main__":
    ...
    async def main():
        await PRO.start()
        # h = await PRO.get_chat_member(chat_id= int(-1002247529229), user_id=7306229091)
        # print(h)
